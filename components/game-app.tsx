  ...
  if (showLoader) {
    return (
      <div className="relative min-h-screen">
        ...
        {!arenaLoaderNoticeAcknowledged && (
          <div
            className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-950/55 backdrop-blur-[2px]"
            role="dialog"
            aria-modal="true"
            aria-labelledby="arena-loader-notice-title"
          >
            <div className="w-full max-w-md rounded-3xl border border-sky-400/35 bg-gradient-to-br from-slate-900 via-slate-900 to-sky-950/40 p-5 shadow-[0_0_0_1px_rgba(56,189,248,0.12),0_20px_50px_rgba(0,0,0,0.45)] pointer-events-auto touch-manipulation">
              ...
              <p className="mt-2 text-xs text-white/50">
                Во ВК окно закроется автоматически через несколько секунд, если кнопка не сработала.
              </p>
              <button
                type="button"
                onClick={acknowledgeArenaLoaderNotice}
                className="mt-5 w-full min-h-[48px] py-3 rounded-xl bg-sky-600 text-white text-sm font-bold hover:bg-sky-500 active:bg-sky-700 transition-colors"
              >
                Хорошо
              </button>
            </div>
